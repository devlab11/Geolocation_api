module IpstackService
  class ErrorHandler < Faraday::Middleware
    def call(env)
      @app.call(env).on_complete do |env|
        ...
        if body_of_error.key?('success')
          ...
          when 500
            raise IpstackService::Api::Exception::InternalServerError,
                  error_message_500(body_of_error, 'Something is technically wrong.')
          end
        end
      end
    end
    ...
    def error_body(body)
      return unless !body.nil? and !body.empty? and body.is_a?(String)

      body = ::JSON.parse(body)
    end

    def error_message_500(response, body = nil)
      "#{response[:method].to_s.upcase} #{response[:url]}: #{[response[:status].to_s + ':',
                                                              body].compact.join(' ')}"
    end
  end
end
